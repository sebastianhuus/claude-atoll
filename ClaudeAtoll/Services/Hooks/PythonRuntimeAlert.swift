...
enum PythonRuntimeAlert {
    ...
    nonisolated private static let logger = Logger(subsystem: "com.engels74.ClaudeAtoll", category: "PythonRuntimeAlert")

    private static func message(for reason: PythonRuntimeDetector.UnavailableReason) -> String {
        switch reason {
        case .noPythonFound:
            """
            Claude Atoll hooks require Python 3.14+ or uv to execute.
            Hooks will not function until a suitable runtime is installed.
            """
        case let .pythonTooOld(version):
            """
            Found Python \(version) but Claude Atoll hooks require Python 3.14+.
