...
# ///
"""Claude Atoll Hook - Session state bridge to ClaudeAtoll.app.

Sends session state to ClaudeAtoll.app via Unix socket.
...
_DEBUG = os.environ.get("CLAUDE_ATOLL_DEBUG") == "1"


def _log(msg: str, /) -> None:
    """Log to stderr when CLAUDE_ATOLL_DEBUG=1. Does not affect stdout JSON."""
    if _DEBUG:
        print(f"[claude-atoll] {msg}", file=sys.stderr, flush=True)
...
class PermissionResponse(TypedDict, total=False):
    """Response from ClaudeAtoll.app for permission requests."""
...
SOCKET_PATH = Path("/tmp/claude-atoll.sock")
...
def handle_permission_response(response: PermissionResponse | None, /) -> None:
    """Handle the permission response from ClaudeAtoll.app.
        ...
        case "deny":
            output = {
                "hookSpecificOutput": {
                    ...
                    "decision": {
                        "behavior": "deny",
                        "message": reason or "Denied by user via ClaudeAtoll",
                    },
                }
            }
